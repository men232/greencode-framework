		...
		if ( bBlockMenu ) then
			...
			if ( TER_ONW:HoldingCount( gc.Client ) < 1 ) then
				...
				CMENU_PLUGIN:ApplyTemplate( BLOCK, "simple", {
					color = Color(200,0,0),
					title = "Внимание",
					desc = greenCode.kernel:TextWarp("Для того, чтобы выбрать другие профессии Вам необходимо купить дом или торговую точку (для торговца/повара). Покупку можно осуществить через F4 -> Собственность в момент нахождения на оной одним из 3-х способов оплаты. Только после покупки жилья или магазина Вам откроется доступ к списку профессий и их выбору. Заметьте, что при продаже единственной собственности Вы будете сняты с занимаемой должности и снова станете обычным гражданином.", w-20, "Default"),
				});
